...
#                stage.eval()
            ...
            if downsamples[i] or embed_dims[i] != embed_dims[i + 1]:
                ...
                embedding_layer = Embedding(
                        ...
                        asub=asub,
                        act_layer=act_layer, norm_layer=norm_layer)
#                network.append( embedding_layer.eval() if i < 2 else embedding_layer)
                network.append(embedding_layer)
...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 1000, 'input_size': (3, 256, 128), 'pool_size': None,
...
def append_prefix_if_present(state_dict: Dict[str, Any], prefix: str) -> None:
    """
...
def build_efficientformerv2_backbone(cfg):
    """
...
#    depth         = cfg.MODEL.BACKBONE.DEPTH
    ...
    configs = {'pretrained_cfg': None, 
            'pretrained_cfg_overlay': None, 
            'num_classes': 1000, 
            'distillation': False, 
            'fuse': False,
            'resolution' : 224}
    model = efficientformerv2_s2(pretrained=pretrain, **configs)

    if pretrain:
        # Load pretrain path if specifically
        if pretrain_path:
            try:
                state_dict = torch.load(pretrain_path, map_location=torch.device('cpu')).pop('model')
        ...
        #append_prefix_if_present(state_dict, 'network.')
